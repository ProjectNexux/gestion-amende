    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">{items.length} facture(s) — reçues automatiquement ou ajoutées manuellement.</p>
        </div>
        <Link
          href="/comptabilite/factures/new"
            ...
            {items.map((item) => {
              ...
              return (
                <tr key={item.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-3">
                    <Link href={`/comptabilite/factures/${item.id}`} className="font-medium text-slate-800 hover:underline">
                      {d.emetteur ?? "Non détecté"}
                    </Link>
                  </td>
              ...
              <tr>
                <td colSpan={9}>
                  <EmptyState
                    ...
                    description="Les factures détectées automatiquement ou ajoutées manuellement apparaîtront ici."
                    action={{ label: "Ajouter une facture", href: "/comptabilite/factures/new" }}
                  />
                </td>
              </tr>
            )}
